...
var firebaseConfig = {
    apiKey: "AIzaSyC7gYuCa_v_lC9mvUV7020baqw5dngcP3o",
    authDomain: "book-shop-project-saar.firebaseapp.com",
    databaseURL: "https://book-shop-project-saar.firebaseio.com",
    projectId: "book-shop-project-saar",
    storageBucket: "book-shop-project-saar.appspot.com",
    messagingSenderId: "456523541415",
    appId: "1:456523541415:web:f70d34ce0aea791850a482"
  };
...
let isAddingNeeded=true;
  ...
  await this.db.ref('Users/'+userDBkey+'/cart').once('value')
  .then((snapshot)=>{
    snapshot.forEach((childSnapshot)=>{
    ...
    if(isAddingNeeded){
      this.db.ref('Users/'+userDBkey+'/cart')
      .push(BookID)
      alert("Added to cart")
  ...
  let isRemovalNeeded=false;
    ...
    .then((snapshot)=>{
      ...
      if(isRemovalNeeded){
        this.db.ref('Users/'+userDBkey+'/cart/'+keyToRemove)
        .remove()
...
console.log(newBook)
 ...
 await this.db.ref('Books/'+newBook.BookID).update(newBook).then(function() {
...
async getBooksObjects(booksIDarray){
  ...
  .then((snapshot) => {
      ...
      }else{//all books will be added to booksObjects
        snapshot.forEach((childSnapshot) => {
          booksObjects.push(childSnapshot.val())
